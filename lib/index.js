        ...
        case 'full-terraform-text-node':
          try {
            ...
            if (error && error.status === 2) {
              throw new Error('Syntax error: ' + error.stderr);
            } else {
              if (opts.terraformStrictError) {
                throw new Error('Unable to format Terraform file: ' + error.stderr);
              } else {
                console.error('Unable to format Terraform file: ' + error.stderr);
              }
            }
            return node.value;
          }
